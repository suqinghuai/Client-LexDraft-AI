...
- 📂 **历史记录** — 每次生成或优化文书后自动保存至本地 history 文件夹，左侧可折叠历史记录栏支持查看详情、载入编辑器、单条删除与一键清空
...
### v1.3.0    ----2026.6.19
- ✨ 新增「历史记录」功能
  - 每次生成文书或优化文书后，自动将记录保存至程序同级目录下 `history/` 文件夹
  - 历史记录以 JSON 格式存储，包含文书名称、生成时间、文书内容
  - 左侧导航栏底部新增可折叠「历史记录」栏目，默认收起，点击展开/折叠
  - 历史记录列表按时间倒序排列，每条显示文书名称、生成时间、删除按钮
  - 点击记录可弹窗查看详情，支持复制内容与载入到编辑器继续编辑
  - 支持单条删除与一键清空全部历史记录（清空前需确认）
- ✨ 左侧导航栏新增「🏠 首页」与「⚙ 系统设置」快捷按钮
  - 首页按钮：一键返回系统首页，查看功能概览与支持的文书类型统计
  - 系统设置按钮：快速进入 API 配置与提示词模板管理页面
- 🎨 重构左侧导航栏布局
  - 采用「先固定底部、再填充中间」的 pack 布局策略，确保历史记录栏与底部按钮始终可见
  - 底部区域固定显示：首页、系统设置按钮
  - 历史记录栏固定在底部按钮上方，可折叠展开/收起
  - 文书分类区域自适应填充剩余空间

### v1.2.1    ----2026.6.16
- ✨ 新增「🔍 智能引导」功能
  - 侧边栏新增智能引导入口，位于智能法律顾问下方
  - 用户用自然语言描述法律需求（如"邻居装修损坏了我家墙壁，我想起诉他赔偿"）
  - 大模型分析需求后，推荐最多3个最匹配的文书类型
  - 每个推荐项显示文书名称、推荐理由、匹配度（高/中/低）
  - 点击「前往草拟」按钮可直接跳转到对应文书编辑页面
  - 未匹配到合适文书时给出友好提示，引导用户手动选择
